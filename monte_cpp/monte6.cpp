...
const char writeFileName[] = "monte.raw";

class csv{
public:
  csv(string filename, int sizex, int sizey);
  string fname="xcom2.csv";
  bool csv_get=false;
  string getfname(){
    return fname;
  }
      ...
      for(i = 0; i <= NUM; i++){
        ...
        if(length>=10){//1回目で検出器到着---------------primary-----------------change10->100
          ...
          for(int a=0; a<ScatterNUM; a++){//相互作用開始
            ...
            if(sc_rand <=  csvarray[2][(int)(Energy+0.5)] / mu){//光電効果
              //cout<<"光電 \n";
              break;
            }
            else if(csvarray[2][(int)(Energy+0.5)] / mu < sc_rand  && sc_rand <= (csvarray[0][(int)(Energy+0.5)]+csvarray[2][(int)(Energy+0.5)])/ mu){
            //コヒーレント散乱----------------------------------------------------------------------------
              //cout<<"coh \n";
...
void writecsv(int height, int width, int* image, int* es, int num){
  // ファイル入力ストリームの初期化
    ofstream ofs(to_string(num)+"monte3.csv");
    ...
    ofstream ofs2(to_string(num)+"es3.csv");
    for(int i=0;i<=140;i++){
      if(es[i] == 0){
        ofs2 << i << "," << 1 <<endl;
      }
