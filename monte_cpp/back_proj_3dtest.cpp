...
const char writeFileName_map[] = "projection_test\\testbp";
const char writeFileName_xy[] = "projection_test\\tes3xy";
const char writeFileName_project[] = "projection_test\\out32d";
...
int main(void){
  ...
  float*	projection_w=(float*)calloc(H, sizeof(float));	/** 重み付け画像用配列 **/
  float*	projection_out=(float*)calloc(H * W, sizeof(float));	/** 出力画像用配列 **/
	FILE	*fpi;	/** ファイルポインタ **/

  //3あり画像か無しかは要確認
	//fpi = fopen( "projection_test\\test2d.raw" , "rb");
	//fread(projection, sizeof(float), H, fpi);
  float*	map=(float*)calloc(H * W, sizeof(float));	/** 原画像用配列 **/
  fpi = fopen( "mapr.raw" , "rb");
	fread(map, sizeof(float), H*W, fpi);

  for(int q = 0; q < 65; q++){
    projection[q] = map[H*32 + q];
  }   
  ...
  for(int zeta = 0; zeta < H; zeta++){
    projection_w[zeta] = projection[zeta]*(32.5/(sqrt(pow(22.5,2)+pow(-16.25+0.5*zeta,2))))/0.5;
  }
  ...
  //重畳積分
     for(int b = 0; b<W; b++){
       double tmp = 0.;
       for(int c = 0; c<W; c++){
         tmp += projection_w[c]*0.5*ramp[H-1-b+c];
         //if(a == 0 && c == 32){cout<< c <<" "<<64-b+c<<endl;}
       }
       projection_out[b] = tmp;
       //map[a*H + b]+=tmp;
     }
  cout<<"filtered"<<endl;
      ...
      for(int t = 10; t < H; t++){//高さ↑ y 15 to H-15
        for(int s = 25; s < W; s++){//幅→ 右半分だけ再構成 x 32 to W
          ...
          if((t-32)*(t-32)+(s-46)*(s-46)<=100){//380
            ...
            double pora = projection_out[int(index_y+0.5)] * weight + projection_out[int(index_y+0.5)-1] * (1-weight);
...
//}
  ...
  writeRawFile(wname_project, sizeof(float), H, projection_out);
  FILE* fp = fopen(wname_project, "wb");
  fwrite(projection_out , sizeof(float), H, fp);
