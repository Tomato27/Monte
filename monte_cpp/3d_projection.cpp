...
int main(void){
/*要3d化start-------------------------------------------------------*/
  int H=65,W=57,num_images=65;
  
  int*	geometry=(int*)calloc(H * W * num_images, sizeof(unsigned char));	/** 原画像用配列 **/
...
int theta_s[181]={0};
  ...
  for(int q=ScatterNUM;q<=ScatterNUM;q++){
    ...
    for(int keV=start_keV;keV<=end_keV;keV++){     
      ...
      int* image = (int*)calloc(H*H, sizeof(int));

      float thtea_base = (60/65)*M_PI;
      float phi_base = (60/65)*M_PI;

      

      //--------------------------------------------------3d_projection
      for(int theta_index=0; theta_index<65;theta_index++){
        for(int phi_index=0; phi_index<65;phi_index++){

          int index_geometry = 16.25*sqrt(3)*N*57+j*0+0;
          //float unit_vec = ()/;

          while(photon.x>=16.25*sqrt(3)){
            image[theta_index*65+phi_index] += geometry[index_geometry+];
          }
