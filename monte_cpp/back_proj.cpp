...
const char writeFileName_xy[] = "projection_test\\tesnxy";
const char writeFileName_project[] = "projection_test\\outp2d";
...
int main(void){
  ...
  for(int zeta = 0; zeta < H; zeta++){
    projection_w[zeta] = projection[zeta]*(32.5/(sqrt(pow(32.5,2)+pow(-16.25+0.5*zeta,2))));
  }
    ...
    //25~29,34
      for(int t = 10; t < H; t++){//高さ↑ y 15 to H-15
        for(int s = 25; s < W; s++){//幅→ 右半分だけ再構成 x 32 to W
          ...
          if((t-32)*(t-32)+(s-46)*(s-46)<=380){//380
            //球領域にのみ逆投影100
            //dbetaをかける
            //Dはオブジェクト中心 (pow(22.5,2)/pow(22.5 - d,2)) * 
            double tmp_output = (pow(22.5,2)/pow(22.5 - d,2)) * projection_out[index_y] * beta_span * 2 * M_PI/360;
