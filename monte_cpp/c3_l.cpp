            ...
            //コヒーレント散乱----------------------------------------------------------------------------
             ...
             //cout<<ditector_index<<endl;
              ...
              if(atan2(p->y,p->x)>0){
                ...
                //phi_a_result = M_PI+atan2(p->y,p->x);
                phi_a_result = atan2(p->y,p->x);
                if(atan2(p->y,p->x)>-M_PI*1/180){
                  ditector_index = 0;
                }
              ...
              if(ditector_index<0||ditector_index>180||phi_a_result>2*M_PI){
              ...
              //phi_a_result = atan2(p->y,p->x);
                ...
                /*if(a!=q-1){
                  if(abs(y_rotate)>10){
                    cout<<x_rotate<<" "<<y_rotate<<" "
                    <<p->x<<" "<<p->y<<" "
                    <<p->x_p<<" "<<p->y_p<<" "
                    <<p->x_p*cos(-phi_check) - p->y_p*sin(-phi_check)<<" "<<p->x_p*sin(-phi_check) + p->y_p*cos(-phi_check)<<" "
                    <<phi_a_result<<" "<<phi_check<<" "<<di_num<<endl;
                  }
                  phi_check = phi_a_result;
                }*/
                ...
                if(abs(y_rotate)>10){
                  cout<<p->x<<" "<<p->y<<" ";
                  if(com_flag ==true){
                    cout<<" check"<<p->x_p<<" "<<p->y_p<<endl;
                  }
                }*/

                com_flag=true;
                if(hypot(p->x,p->y)>10&&x_rotate<10){
                  cout<<p->x<<" "<<p->y<<" "<<x_rotate<<" "<<y_rotate<<" "
                  <<phi_a_result<<" "
                  <<endl;
                }
                ...
                //検出器を通過したかcheck,今回は必ず検出器まで到達する
                  ...
                  count++;
                  es[(int)(Energy+0.5)]++;
                  //cout<<"d_y: "<<ditector_y<<endl;

                  if(abs(ditector_y)>10){
                    cout<<"com: "<<p->x_p<<" "<<p->y_p<<" "
                    <<x_p_rotate<<" "<<y_p_rotate<<" "
                    //<<hypot(p->x_p,p->y_p)<<" "
                    <<p->x<<" "<<p->y<<" "
                    <<x_rotate<<" "<<y_rotate<<" "
                    //<<ditector_index<<" "<<phi_a_result
                    <<" "<<di_num<<
                    endl;
...
void delta_sampling(photon* p, double mu_H2O, double mu_Ca,int* geometry, double sin_theta_a, double cos_theta_a, double sin_phi_a, double cos_phi_a){
  ...
  while(loop_flag){
    ...
    if(geometry[65*(int)(y*2+32.5)+(int)(x*2+32.5)]==0){//空気の時，直進し続ける
      if(sqrt(x*x+y*y)>10+2.5){//sin1度の分軽く足しておく 5*sqrt(5)+0.1
        loop_flag = false;
      }
    }
