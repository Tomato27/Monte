...
const char writeFileName[] = "monte_ca0.raw";
...
int main(void){
  ...
  double** csvarray = new double*[sizex];
  for(int k=0;k<sizex;k++){
    csvarray[k]=new double[sizey+5];
  }
...
int theta_s[181]={0};
  ...
  /*csv読み込み*/
  csv csv_xcom("Ca.csv", 4, 200);
  readcsv(csv_xcom, 4, 200, csvarray);
  ...
  for(int keV=start_keV;keV<=end_keV;keV++){
      ...
      double mu=csvarray[3][(int)(Energy+0.5)]*dens_H2O;
      ...
      for(i = 0; i <= NUM; i++){
        ...
        if(length>=10){//1回目で検出器到着---------------primary-----------------change10->100
          primary_num++;
          /*count++;
          image[H*32+32]++;
          es[H*32+32]++;*/
          ...
          for(int a=0; a<ScatterNUM; a++){//相互作用開始
            ...
            if(sc_rand <=  csvarray[2][(int)(Energy+0.5)]*dens_H2O / mu){//光電効果
              //cout<<"光電 \n";
              break;
            }
            else if(csvarray[2][(int)(Energy+0.5)]*dens_H2O / mu < sc_rand  && sc_rand <= (csvarray[0][(int)(Energy+0.5)]+csvarray[2][(int)(Energy+0.5)])*dens_H2O/mu ){
            //コヒーレント散乱----------------------------------------------------------------------------
              //cout<<"coh \n";
              ...
              float xp= p->before_vec[0]*length;
              float yp= p->before_vec[1]*length;
              float zp= p->before_vec[2]*length;
              ...
              double v_length = sqrt(pow(length*sin_theta_a_new*cos_phi_a_new,2) + pow(length*sin_theta_a_new*sin_phi_a_new,2) +pow(length*cos_theta_a_new,2));
              p->before_vec[0] = length*sin_theta_a_new*cos_phi_a_new / v_length;
              p->before_vec[1] = length*sin_theta_a_new*sin_phi_a_new / v_length;
              p->before_vec[2] = length*cos_theta_a_new / v_length;
              //vectorは長さ1に正規化
...
void writecsv(int height, int width, int* image, int* es, int num){
  // ファイル入力ストリームの初期化
    ofstream ofs(to_string(num)+"monte_ca.csv");
    ...
    ofstream ofs2(to_string(num)+"es_ca.csv");
    for(int i=0;i<=140;i++){
      if(es[i] == 0){
        ofs2 << i << "," << 1 <<endl;
      }
