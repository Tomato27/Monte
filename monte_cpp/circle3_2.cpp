      ...
      for(i = 0; i <= NUM; i++){
        ...
        if(p->x*p->x+p->y*p->y>=10*10){//1回目で検出器到着---------------primary-----------------change10->100
          ...
          for(int a=0; a<q; a++){//相互作用開始
            ...
            else if(ab / mu+8000000 < sc_rand  && sc_rand <= (ab + coh)/mu){
             ...
             //cout<<ditector_index<<endl;
              ...
              //cout<<ditector_index<<" "<<phi_a_result<<endl;
                ...
                //検出器を通過したかcheck,今回は必ず検出器まで到達する
                  ...
                  if(abs(ditector_y)>10){
                    cout<<phi_a_result<<" "<<ditector_index<<endl;
                    //cout<<"com"<<ditector_y<<" "<<ditector_index<<" "<<phi_a_result<<" "<<com_flag<<" "<<coh_flag <<endl;
                    //cout<<Energy<<endl;
                  }
