...
#define FNAME(a) "circle\\" #a "_" ".raw"


using namespace std;

const char writeFileName[] = "circle\\monte_c_";
const char writeFileName1[] = "circle\\monte_c_1.raw";
const char writeFileName2[] = "circle\\monte_c_2.raw";
const char writeFileName3[] = "circle\\monte_c_3.raw";
const char writeFileName4[] = "circle\\monte_c_4.raw";
const char writeFileName5[] = "circle\\monte_c_5.raw";
...
int theta_s[181]={0};
  ...
  for(int keV=start_keV;keV<=end_keV;keV++){
      ...
      for(i = 0; i <= NUM; i++){
        ...
        double cos_theta_a=1., cos_phi_a=1., sin_theta_a=0., sin_phi_a=0.;
...
*/
              ...
              p->before_vec[0]=(p->x-p->x_p)/sqrt(pow(p->x_p,2)+pow(p->y_p,2)+pow(p->z_p,2));
              p->before_vec[1]=(p->y-p->y_p)/sqrt(pow(p->x_p,2)+pow(p->y_p,2)+pow(p->z_p,2));
              p->before_vec[2]=(p->z-p->z_p)/sqrt(pow(p->x_p,2)+pow(p->y_p,2)+pow(p->z_p,2));
              ...
              if(p->z<0){
                ...
                else if(abs(ditector_x) <=16.25 && abs(ditector_y)<=16.25 && a == q-1){
      ...
      sprintf(wname,"%s%d.raw",writeFileName, q);
        ...
        writeRawFile(wname, sizeof(int), H * W, image);

        FILE* fp = fopen(wname, "wb");
        fwrite(image, sizeof(int), H*W, fp);
        fclose(fp);
        writecsv(32, W, image, es,q);
...
void writecsv(int height, int width, int* image, int* es, int num){
  // ファイル入力ストリームの初期化
    ofstream ofs("circle\\"+to_string(num)+"monte_c.csv");
    ...
    ofstream ofs2("circle\\"+to_string(num)+"es_c.csv");
    for(int i=0;i<=140;i++){
      if(es[i] == 0){
        ofs2 << i << "," << 1 <<endl;
      }
