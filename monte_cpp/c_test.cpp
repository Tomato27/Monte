...
const char writeFileName[] = "circle_result\\monte_c_";
          ...
          for(int a=0; a<q; a++){//相互作用開始
            ...
            //コヒーレント散乱----------------------------------------------------------------------------
              ...
              if(com_flag==false){
                ...
                p_r = phi_a_result;
                cos_check = cos_phi_a_new;          
              }

              x_rotate = p->x*cos(-p_r) - p->y*sin(-p_r);
              y_rotate = p->x*sin(-p_r) + p->y*cos(-p_r);            
              x_p_rotate = p->x_p*cos(-p_r) - p->y_p*sin(-p_r);
              y_p_rotate = p->x_p*sin(-p_r) + p->y_p*cos(-p_r);

              ditector_y=(10-x_p_rotate)*(y_rotate-y_p_rotate)/(x_rotate-x_p_rotate)+y_p_rotate;
             ...
             //cout<<ditector_index<<endl;
              ...
              delta_sampling(p, mu_H2O, mu_Ca, geometry, sin_theta_a_new, cos_theta_a_new, sin_phi_a_new, cos_phi_a_new);
              //if(com_flag ==true)cout<<x_check<<" "<<y_check<<" "<<p->x_p<<" "<<p->y_p<<endl;
              ...
              int di_num=0;
              if(atan2(p->y,p->x)>0){
                ditector_index = 180*atan2(p->y,p->x)/(2*M_PI)+0.5;
                //phi_a_result = atan2(p->y,p->x);
                if(atan2(p->y,p->x)<M_PI*1/180){
                  ditector_index = 0;
                  //phi_a_result = 0;
                }
              }
              ...
              //cout<<" "<<"transrated ";
                ...
                if(180*asin(sin_phi_a_new)/(2*M_PI)>0){
              ...
              if(ditector_index<0){
               ...
               double x_rotate=0, y_rotate=0, x_p_rotate=0, y_p_rotate=0;
                ...
                com_flag=true;
                /*if(hypot(p->x,p->y)>10&&x_rotate<10){
                  cout<<p->x_p<<" "<<p->y_p<<" "<<x_rotate<<" "<<y_rotate<<" "
                  <<phi_a_result<<" "
                  <<endl;
                }*/


                if(x_rotate>=10){//hypot(p->x,p->y)>10
                //検出器を通過したかcheck,今回は必ず検出器まで到達する

                  if(a != q-1){
                    //cout<<p->x<<" "<<p->y<<" "<<x_rotate<<" "<<y_rotate<<endl;
                    break;
                  }
                  
                  ditector_y=(10-x_p_rotate)*(y_rotate-y_p_rotate)/(x_rotate-x_p_rotate)+y_p_rotate;//x==10の時のy座標
...
void writecsv(int height, int width, int* image, int* es, int num){
  // ファイル入力ストリームの初期化
    ofstream ofs("circle_result\\"+to_string(num)+"monte_c.csv");
    ...
    ofstream ofs2("circle_result\\"+to_string(num)+"es_c.csv");
    cout<<"es \n";
    for(int i=0;i<=140;i++){
      if(es[i] == 0){
        ofs2 << i << "," << 1 <<endl;
...
void delta_sampling(photon* p, double mu_H2O, double mu_Ca,int* geometry, double sin_theta_a, double cos_theta_a, double sin_phi_a, double cos_phi_a){
  ...
  while(loop_flag){
    ...
    /*if(x*x+y*y>100){//空気の時，直進し続ける
      if(nu <=  mu_H2O/mu_max){
        loop_flag = false;
        //break;
      }
    }
    else{//if(geometry[65*(int)(y*2+32.5)+(int)(x*2+32.5)]==0){//空気の時，直進し続ける
    //hypot(x,y)<=10 
      if(sqrt(x*x+y*y)>20){//sin1度の分軽く足しておく 5*sqrt(5)+0.1
        loop_flag = false;
      }
    }
