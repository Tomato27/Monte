...
const char writeFileName[] = "CBCTtest3\\monte_f";
...
int theta_s[181]={0};
  ...
  for(int q=ScatterNUM;q<=ScatterNUM;q++){
    for(int keV=start_keV;keV<=end_keV;keV++){     
      ...
      for(i = 0; i < H; i++){//theta縦
        for(int j = 0;j < W; j++){//phi横
          for(int k  = 0;k < per;k++){
        ...
        p = new photon;
        double theta_sum = 0.;
        p -> x = -16.25;//-16.25*sqrt(3)/2;
        p -> y = 0;
        p -> z = 0;

        double  xl = 16.25, yl = 16 - 0.5 * i, zl = 16 - 0.5 * j;

        double cos_phi_a, sin_phi_a;
        double theta_a = 0.5*M_PI - atan(zl/32.5);
        //cout<<theta_a*180/M_PI<<" ";
        //if(k==0&&i==0)cout<<theta_a*180/M_PI<<" ";
        //theta_a = angle_t[i]; 
        //theta_a=M_PI/3;
        ...
        if(p->x>=16.25){//1回目で検出器到着---------------primary-----------------&&yの範囲
          primary_num++;
          if(q==0){
            count++;
            //image[H*32+32]++;
            ...
            double d_y = (p->y/(p->x+p_))*2*p_;//ok?
            
            int result_y = -1*(int(d_z*2-32.5));
            int result_x = -1*(int(d_y*2-32.5));
            //if(k==0)cout<<i<<","<<j<<":"<<p->x<<" "<<p->y<<" "<<p->z<<" "<<d_z<<" "<<d_y<<" "<<result_y<<" "<<result_x<<endl;
...
void delta_sampling(photon* p, double mu_H2O, double mu_Ca,unsigned char* geometry, double sin_theta_a, double cos_theta_a, double sin_phi_a, double cos_phi_a){
  ...
  while(loop_flag){
    ...
    if(abs(x)>=16.25||abs(y)>=16.25||abs(z)>=16.25){
      ...
      if(((x+16.25)-23.25)*((x+16.25)-23.25)+((y+16.25)-16.25)*((y+16.25)-16.25)+((z+16.25)-16.25)*((z+16.25)-16.25)<=25){
        check = 1;
      }
      //check=0;//-----------------------------------------
    }
